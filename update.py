...
headerReplace = """const { useState, useCallback, useMemo, useRef, useEffect, useContext, createContext } = React;
...
# HomePage
home_match = re.search(r"function HomePage\(\s*\{\s*onNavigate\s*\}\s*\)\s*\{([\s\S]*?)return\s*\(([\s\S]*?)\);\s*\}", content)
content = content.replace(home_match.group(0), """function HomePage({ onNavigate }) {
...
# PythagoreanCalc
calc_match = re.search(r"function PythagoreanCalc\(\s*\{\s*onDraw\s*\}\s*\)\s*\{([\s\S]*?)\n\s*return\s*\(([\s\S]*?)\);\s*\}", content)
content = content.replace(calc_match.group(0), f"""function PythagoreanCalc({{ onDraw }}) {{
  const t = useTranslation();{calc_match.group(1)}
  return (
...
content = content.replace(
  "setMsg({ type: 'error', text: 'Please fill exactly two fields.' });",
  "setMsg({ type: 'error', text: t('fill_two_fields') });"
).replace(
  "setMsg({ type: 'error', text: 'Invalid values. Check inputs.' });",
...
# TripleIdentifier
triple_match = re.search(r"function TripleIdentifier\(\s*\{\s*onDraw\s*\}\s*\)\s*\{([\s\S]*?)\n\s*return\s*\(([\s\S]*?)\);\s*\}", content)
content = content.replace(triple_match.group(0), f"""function TripleIdentifier({{ onDraw }}) {{
  const t = useTranslation();{triple_match.group(1)}
  return (
...
content = content.replace(
  "setMsg({ type: 'error', text: 'Enter three integers.' });",
...
# Calc454590
calc45_match = re.search(r"function Calc454590\(\s*\{\s*onDraw\s*\}\s*\)\s*\{([\s\S]*?)\n\s*return\s*\(([\s\S]*?)\);\s*\}", content)
content = content.replace(calc45_match.group(0), f"""function Calc454590({{ onDraw }}) {{
  const t = useTranslation();{calc45_match.group(1)}
  return (
...
content = content.replace(
  "setMsg({ type: 'error', text: 'Enter a positive number.' });",
...
ra_match = re.search(r"function RightAngleIdentifier\(\s*\{\s*onDraw\s*\}\s*\)\s*\{([\s\S]*?)\n\s*return\s*\(([\s\S]*?)\);\s*\}", content)
content = content.replace(ra_match.group(0), f"""function RightAngleIdentifier({{ onDraw }}) {{
  const t = useTranslation();{ra_match.group(1)}
  return (
...
content = content.replace(
  "setMsg({ type: 'error', text: 'Enter three positive numbers.' });",
...
# SpiralCreator
spiral_match = re.search(r"function SpiralCreator\(\s*\{\s*onDrawSpiral\s*\}\s*\)\s*\{([\s\S]*?)\n\s*return\s*\(([\s\S]*?)\);\s*\}", content)
content = content.replace(spiral_match.group(0), f"""function SpiralCreator({{ onDrawSpiral }}) {{
  const t = useTranslation();{spiral_match.group(1)}
  return (
...
content = content.replace(
  "setMsg({ type: 'error', text: 'Enter positive values.' });",
...
# SidePanel signature
sidepanel_sig = re.search(r"function SidePanel\(\{\s*activeTab,[\s\S]*?onImport\s*\}\)\s*\{", content)
if sidepanel_sig:
  content = content.replace(sidepanel_sig.group(0), """function SidePanel({ activeTab, setActiveTab, planeSize, setPlaneSize, gridSize, setGridSize, gridSnap, setGridSnap, theme, setTheme, onDrawTriangles, onDrawSpiral, graphActive, onExport, onImport, lang, setLang }) {
  const t = useTranslation();""")
...
content = content.replace('<div className="panel-header"><h2>Calculators</h2></div>', '<div className="panel-header"><h2>{t(\'calculators\')}</h2></div>')

content = content.replace('<h3>Export Design</h3>', '<h3>{t(\'export_design\')}</h3>')
content = content.replace('Save your current grid configuration and triangles.', '{t(\'export_desc\')}')
content = re.sub(r'>\s*Export as \.SVG\s*</button>', '>{t(\'export_svg\')}</button>', content)

content = content.replace('<h3 style={{ marginTop: \'24px\' }}>Import Design</h3>', '<h3 style={{ marginTop: \'24px\' }}>{t(\'import_design\')}</h3>')
content = content.replace('Load a .SVG file to add shapes to your graph.', '{t(\'import_desc\')}')
content = re.sub(r'Import \.SVG\s*<input', '{t(\'import_svg\')}<input', content)

content = content.replace('<h3>Settings</h3>', '<h3>{t(\'settings\')}</h3>')
content = content.replace('<label>Grid Size</label>', '<label>{t(\'grid_size\')}</label>')
content = content.replace('<label>Grid Snap</label>', '<label>{t(\'grid_snap\')}</label>')
content = content.replace('<label>Plane Size</label>', '<label>{t(\'plane_size\')}</label>')

content = content.replace('<option value={0.5}>0.5 units</option>', '<option value={0.5}>{t(\'units_0_5\')}</option>')
content = content.replace('<option value={1}>1 unit</option>', '<option value={1}>{t(\'units_1\')}</option>')
content = content.replace('<option value={5}>5 units</option>', '<option value={5}>{t(\'units_5\')}</option>')
content = content.replace('<option value={10}>10 units</option>', '<option value={10}>{t(\'units_10\')}</option>')

content = content.replace('<label>Color Scheme</label>', '<label>{t(\'color_scheme\')}</label>')
content = content.replace('<option value="pastel-petals">Pastel Petals</option>', '<option value="pastel-petals">{t(\'pastel_petals\')}</option>')
content = content.replace('<option value="olive-wood">Olive Wood</option>', '<option value="olive-wood">{t(\'olive_wood\')}</option>')
content = content.replace('<option value="ocean-blue">Ocean Blue</option>', '<option value="ocean-blue">{t(\'ocean_blue\')}</option>')
content = content.replace('<option value="modern-palette">Modern Palette</option>', '<option value="modern-palette">{t(\'modern_palette\')}</option>')
content = content.replace('<option value="watermelon-sugar">Watermelon Sugar</option>', '<option value="watermelon-sugar">{t(\'watermelon_sugar\')}</option>')
content = content.replace('<option value="pastel-serenity">Pastel Serenity</option>', '<option value="pastel-serenity">{t(\'pastel_serenity\')}</option>')
...
# TriangleInfoPanel
content = re.sub(
  r'function TriangleInfoPanel\(\{\s*triInfo,\s*index,\s*color,\s*onDelete,\s*onDeleteAll\s*\}\)\s*\{',
  "function TriangleInfoPanel({ triInfo, index, color, onDelete, onDeleteAll }) {\n  const t = useTranslation();",
  content
)
content = content.replace('<p>Click a shape on the graph to view its details.</p>', '<p>{t(\'click_shape\')}</p>')
content = content.replace('>Delete All Shapes</button>', '>{t(\'delete_all\')}</button>')
content = content.replace(
...
content = content.replace("<h3>{shapeName} #{index + 1}</h3>", "<h3>{t('shape_index', { name: shapeName, index: index + 1 })}</h3>")
content = content.replace("<h4>Vertices</h4>", "<h4>{t('vertices')}</h4>")
content = content.replace("<h4>Side Lengths</h4>", "<h4>{t('side_lengths')}</h4>")
content = content.replace("<h4>Measurements</h4>", "<h4>{t('measurements')}</h4>")
content = content.replace('<span className="tri-info-label">Perimeter</span>', '<span className="tri-info-label">{t(\'perimeter\')}</span>')
content = content.replace('<span className="tri-info-label">Area</span>', '<span className="tri-info-label">{t(\'area\')}</span>')
content = content.replace("{fmt(area)} sq units", "{fmt(area)} {t('sq_units')}")
content = content.replace(">Delete {shapeName}</button>", ">{t('delete_shape', { name: shapeName })}</button>")


# App -> MainApp
content = re.sub(r'function App\(\)\s*\{', "function MainApp({ lang, setLang }) {", content)
content = content.replace("const [theme, setTheme] = useState('pastel-petals');", "const [theme, setTheme] = useState('pastel-petals');\n  const t = useTranslation();")
content = content.replace('alert("No valid shapes found in the SVG file.");', 'alert(t(\'no_valid_shapes\'));')
content = content.replace('<h1 className="graph-title">Theorem De Pythagoras</h1>', '<h1 className="graph-title">{t(\'app_title\')}</h1>')
...
# App Root
content = re.sub(r"ReactDOM\.createRoot\(document\.getElementById\('root'\)\)\.render\(<App />\);", "", content)
